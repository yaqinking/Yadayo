...
#import "YDExportViewController.h"
...
@interface YDExportViewController ()

@property (nonatomic, strong) NSURL *documentsURL;
@property (nonatomic, copy) NSArray<YDImage *> *cachedImages;
...
@implementation YDExportViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.sectionDatas = @[@"Image Quality", @"Tag"];
    self.imageQulities = @[@"Sample", @"JPEG", @"File"];
    self.tags = [[YDCoreDataStackManager sharedManager] tags];
        ...
        [exportImageQulities enumerateObjectsUsingBlock:^(NSString * _Nonnull imageQuality, NSUInteger idx, BOOL * _Nonnull stop) {
            [exportTags enumerateObjectsUsingBlock:^(NSString * _Nonnull tag, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                if ([tag isEqualToString:@"post"] || [tag isEqualToString:@"all"]) {
                    cachedImages = [[YDCoreDataStackManager sharedManager] cachedImages];
                } else {
                    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"tags CONTAINS %@", tag];
                    cachedImages = [[YDCoreDataStackManager sharedManager] cachedImagesUsingPredicate:predicate];
                }
...
- (NSArray<NSString *> *)cachedImageKeysFrom:(NSArray<YDImage *> *)images WithKey:(NSString *)key {
    NSMutableArray *imageKeys = [NSMutableArray new];
    [images enumerateObjectsUsingBlock:^(YDImage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSURL *)documentsURL {
    if (!_documentsURL) {
        _documentsURL = [[YDCoreDataStackManager sharedManager] applicationDocumentsDirectory];
    }
...
- (NSArray<YDImage *> *)cachedImages {
    if (!_cachedImages) {
        _cachedImages = [[YDLocalImageDataSource alloc] imagesWithTag:@""];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:YDExportCellIdentifier forIndexPath:indexPath];
    YDTag *tag;
